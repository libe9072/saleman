	...
	(function (root, factory) {
		...
		return function download(data, strFileName, strMimeType) {
			var self = window, // this script is only for browsers anyway...
				...
				myBlob =
					self.Blob || self.MozBlob || self.WebKitBlob || toString,
			...
			if (/^data:([\w+-]+\/[\w+.-]+)?[,;]/.test(payload)) {
				if (
					payload.length > 1024 * 1024 * 1.999 &&
					myBlob !== toString
				) {
	...
	methods: {
					...
					if (
						...
						xlsData +=
							...
							_self.preprocessLongNum(
								_self.valueReformattedForMultilines(
									item[key]["val"]
								)
							) +
							"</td>";
		...
		getProcessedJson(data, header) {
			...
			data.map(function (item, index) {
				...
				for (let label in keys) {
					...
					newItem[label] = {
						type:
							typeof property["type"] === "undefined"
								? null
								: property["type"],
						val: _self.getValue(property["val"], item),
					};
				}
				newData.push(newItem);
			});
		...
		*/
		...
		getValue(key, item) {
			...
			if (typeof key !== "undefined" && key.hasOwnProperty("callback"))
				value = this.getValueFromCallback(value, key.callback);
